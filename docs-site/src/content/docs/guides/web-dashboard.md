...
| **Windows tray** | Install a per-user login tray for one-click proxy start, stop, restart, dashboard access, and status. The MOMO Windows installer now installs it by default. The tray is a controller, not a proxy restart service. |
