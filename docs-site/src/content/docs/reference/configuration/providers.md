...
| `momoModelAutoSync?` | `{ enabled?, catalogMode?, intervalMinutes?, autoCreateCombos?, autoRefreshCatalog?, includeImageModels?, exposeUnknownModels?, notifyOnChanges?, managedModelIds? }` | off unless `ocx momo setup` enables it | MOMO Switch background importer. Setup queries the authenticated `/agent/catalog` first and falls back to `/v1/models` for older deployments, enables a 60-minute poll and MOMO-only catalog policy, and projects text models directly under their public ids without creating one-target Combos. Server-advertised reasoning efforts and defaults are authoritative; missing, invalid, or explicitly unsupported capability metadata hides the effort control instead of using installer guesses. Catalog refresh keeps MOMO image models out of the text picker while retaining the MOMO image relay. Native ChatGPT image requests remain on the native OpenAI sidecar; set `catalogMode: "mixed"` only when the general OpenCodex native catalog is intentionally wanted. `managedModelIds` is runtime-owned diff state. |
