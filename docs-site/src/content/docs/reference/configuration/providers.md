...
| `momoModelAutoSync?` | `{ enabled?, catalogMode?, intervalMinutes?, autoCreateCombos?, autoRefreshCatalog?, includeImageModels?, exposeUnknownModels?, notifyOnChanges?, managedModelIds? }` | off unless `ocx momo setup` enables it | MOMO Switch background importer for `https://momoapi.us/v1/models`. Setup enables a 60-minute poll and MOMO-only catalog policy, computes added/removed models, creates aliases for new text models, refreshes the Codex catalog/cache after changes, and keeps MOMO image models on the MOMO image relay. Native ChatGPT image requests remain on the native OpenAI sidecar; set `catalogMode: "mixed"` only when the general OpenCodex native catalog is intentionally wanted. `managedModelIds` is runtime-owned diff state. |
