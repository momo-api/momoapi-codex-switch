...
| `momoModelAutoSync?` | `{ enabled?, catalogMode?, intervalMinutes?, autoCreateCombos?, autoRefreshCatalog?, includeImageModels?, exposeUnknownModels?, notifyOnChanges?, managedModelIds? }` | off unless `ocx momo setup` enables it | MOMO Switch background importer for `https://momoapi.us/v1/models`. Setup fetches the live roster immediately, enables a 60-minute poll and MOMO-only catalog policy, projects text models directly under their public ids without creating one-target Combos, refreshes the Codex catalog/cache after changes, and keeps MOMO image models out of the text picker while retaining the MOMO image relay. Native ChatGPT image requests remain on the native OpenAI sidecar; set `catalogMode: "mixed"` only when the general OpenCodex native catalog is intentionally wanted. `managedModelIds` is runtime-owned diff state. |
