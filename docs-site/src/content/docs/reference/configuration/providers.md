...
| `momoModelAutoSync?` | `{ enabled?, intervalMinutes?, autoCreateCombos?, autoRefreshCatalog?, includeImageModels?, exposeUnknownModels?, notifyOnChanges? }` | off unless `ocx momo setup` enables it | MOMO Switch background importer for `https://momoapi.us/v1/models`. The setup flow enables it with a 60-minute interval, creates short combo aliases for new text models, refreshes the Codex catalog after changes, and keeps image-generation models on the image relay instead of the chat model picker. |
