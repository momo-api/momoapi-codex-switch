...
function googleKeyUsesBearer(provider: OcxProviderConfig): boolean {
  return provider.googleApiKeyTransport === "bearer";
}
...
export function createGoogleAdapter(provider: OcxProviderConfig): ProviderAdapter {
  ...
  // Direct Gemini-compatible gateways can enforce Gemini 3 thought-signature
  // replay too. Keep their cache namespace distinct from Vertex/CCA, while
  // using Codex's stable thread id when the client provides one.
  ...
  return {
    ...
    async buildRequest(parsed: OcxParsedRequest) {
      ...
      // AI Studio uses x-goog-api-key. Google-compatible gateways may opt into
      // Bearer auth without pretending to be Vertex or changing the wire path.
      ...
      if (googleKeyUsesBearer(provider)) headers["Authorization"] = `Bearer ${apiKey}`;
      else headers["x-goog-api-key"] = apiKey;

      const compiled = compileGoogleWireBody(body);
      restoreGoogleToolName = compiled.restoreToolName;
      directReplayModel = `direct:${provider.baseUrl}:${routedModelId}`;
      ...
      const handleDataLine = async function* (line: string): AsyncGenerator<AdapterEvent, "continue" | "content" | "terminate"> {
        ...
        if (chunk.error) {
          ...
          const replayModel = provider.googleMode === "cloud-code-assist"
            ? antigravityModel
            : provider.googleMode === "vertex" ? vertexReplayModel : directReplayModel;
          const replaySession = provider.googleMode === "cloud-code-assist"
            ? antigravitySession
            : provider.googleMode === "vertex" ? vertexReplaySession : directReplaySession;
          if (replayModel && replaySession
        ...
        const replaySession = provider.googleMode === "cloud-code-assist"
          ? antigravitySession
          : provider.googleMode === "vertex" ? vertexReplaySession : directReplaySession;
        if (parts && replayModel && replaySession) {
