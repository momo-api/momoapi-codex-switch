...
describe("install scripts", () => {
  ...
  test("POSIX installer matches the Node launcher prerequisite", async () => {
    ...
    expect(script).toContain("npm install -g @momo-api/momoapi-codex-switch");
    expect(script).toContain("command -v ocx");
    expect(script).toContain("ocx help");
    expect(script).not.toContain("bun install -g @momo-api/momoapi-codex-switch");
  ...
  test("PowerShell installer matches the Node launcher prerequisite", async () => {
    ...
    expect(script).toContain("Node.js 18+ is required");
    expect(script).toContain("& $npm.Source install -g @momo-api/momoapi-codex-switch");
  ...
  test("MOMO Windows installer initializes and passes an explicit Codex home", async () => {
    ...
    expect(script).toContain("server-advertised reasoning capabilities");
